...
export default function WriteTab({ modelReady, selectedModel }: Props) {
    ...
    const listener = (message: ExtensionMessage) => {
      if (message.type === MessageType.CONTEXT_MENU_ACTION) {
        ...
        if (modelReady) {
          ...
          const customCmd = commands.find(c => c.id === message.payload.action);
          const messages = customCmd
            ? buildWriteMessagesCustom(message.payload.text, customCmd.prompt)
            : buildWriteMessages(message.payload.text, message.payload.action);
  ...
  }, [modelReady, generate, onComplete, commands, selectedModel]);
  ...
  const handleGetSelection = async () => {
    ...
    try {
      ...
      if (response?.text) {
        ...
        setFetchError('No text selected. Select text on any page first.');
      }
    } catch {
      setFetchError('Failed to get selection. Try refreshing the page.');
    }
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-b border-gray-200 dark:border-gray-700 space-y-2">
        ...
        <textarea
          ...
          placeholder="Paste or select text to transform..."
          rows={4}
        ...
        {displayText ? (
          <>
            ...
            {!isGenerating && displayText && (
              <button
                onClick={handleCopy}
                className="mt-2 py-1 px-3 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-md transition-colors"
              >
          ...
          <p className="text-center text-gray-400 dark:text-gray-600 text-sm mt-8">
            Select text on a page or paste text above, then choose an action
          </p>
        )}
      </div>
