...
export default function ChatTab({ modelReady, selectedModel }: Props) {
  const { messages, addMessage, clearAll, isLoaded } = useChatStorage();
  const [input, setInput] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [searchError, setSearchError] = useState('');

  const { streamingText, isGenerating, tps, error, onComplete: _onComplete, generate, interrupt } = useInference();
